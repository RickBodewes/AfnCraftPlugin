...
public class SwordOldGuard extends EntitySkeleton{
	public SwordOldGuard(Location loc) {
		super(EntityTypes.SKELETON, ((CraftWorld) loc.getWorld()).getHandle());
		this.setPosition(loc.getX(), loc.getY(), loc.getZ());
		
		//setting equipment
		this.setSlot(EnumItemSlot.MAINHAND, CraftItemStack.asNMSCopy(OldGuardsWeapons.oldGuardSword));
		this.setSlot(EnumItemSlot.OFFHAND, CraftItemStack.asNMSCopy(OldGuardsWeapons.oldGuardShield));
		...
		//setting params
		this.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED).setValue(.22);
